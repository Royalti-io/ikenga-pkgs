# Held changesets — @ikenga/skill-research

These changesets are **intentionally held out of `.changeset/`** so the release
workflow does not publish `@ikenga/skill-research` yet.

Per the launch plan: skill-research is held off the catalog until it has its own
publish WP, which is now just a marketplace entry — the 3-copy mirror model was retired
(see ikenga-pkgs#66).

`@ikenga/skill-research` is also marked `"private": true` in its `package.json` —
