...
// Copied verbatim from tasks/dist/lib/bridge.js and rebranded for com.ikenga.mail.
//
...
export async function connectBridge({ name, version, onContextChange }) {
  ...
  app.onerror = (err) => console.error('[mail] bridge error', err);
...
export async function hostDbQuery(sql, params = []) {
  if (!app) throw new Error('[mail] bridge not connected — db_query unavailable');
...
export async function hostDbExec(sql, params = []) {
  if (!app) throw new Error('[mail] bridge not connected — db_exec unavailable');
  const res = await app.callServerTool({
    name: 'host.dbExec',
    arguments: { sql, params },
  });
...
export async function hostSendToActiveSession(prompt, source = 'com.ikenga.mail') {
  if (!app) throw new Error('bridge not connected');
