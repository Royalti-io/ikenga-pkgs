// Sales root — bridge → mount <SalesView/>.
//
...
//            plans/atelier-design-system/parts/screens/sales.md §§1–4
// WP-18b: builds Pipeline (list+kanban) / Forecast / Won views; owns 0043 migration.
...
import { SalesView } from './features/sales/sales-view.js';
import tokensCss from './lib/tokens-css.js';
import appKitCss from './lib/app-kit-css.js';
import salesCss from './lib/sales-css.js';

// ─── CSS injection ────────────────────────────────────────────────────────────
// Inject order: tokens → app-kit → sales residue. <link>/@import fail inside
...
injectCss('data-sales-css', salesCss);
...
// theme/tint/system-OS flips. data-workspace="sessions" = warm amber-ochre tint
// for Sales (sessions-era pipeline tool per the screen doc §2).
...
// Pre-set data-workspace="sessions" so the sessions tint renders immediately
// without a flash — overwritten at runtime by setupTheme() if shell provides it.
root.setAttribute('data-workspace', 'sessions');
setupTheme();
