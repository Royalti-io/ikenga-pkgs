// Research root — bridge → mount <ResearchView/>.
//
...
//            plans/atelier-design-system/parts/screens/research.md §§1–4
// WP-22b: builds Reports (list+detail) / Sources (monitored register) / Personas (ICP grid);
//         owns 0053 migration (research_notes ext cols, research_sources, sales_deals.research_item_id).
...
import { ResearchView } from './features/research/research-view.js';
import tokensCss from './lib/tokens-css.js';
import appKitCss from './lib/app-kit-css.js';
import researchCss from './lib/research-css.js';

// ─── CSS injection ────────────────────────────────────────────────────────────
// Inject order: tokens → app-kit → research residue. <link>/@import fail inside
...
injectCss('data-research-css', researchCss);
...
// theme/tint/system-OS flips. data-workspace="agents" = teal tint for Research
// (agents tint per the screen doc §2).
...
// Pre-set data-workspace="agents" so the teal tint renders immediately
// without a flash — overwritten at runtime by setupTheme() if shell provides it.
root.setAttribute('data-workspace', 'agents');
setupTheme();
...
function App() {
  ...
  return html`<${QueryClientProvider} client=${queryClient}><${ResearchView} activeFeature=${activeFeature} operatorId=${operatorId} operatorLabel=${operatorLabel} /></${QueryClientProvider}>`;
}
