// COPY-OF: plans/git/drafts/rpc.ts (G-RPC, frozen). WP-06 UI-side copy.
// DO NOT hand-edit this file's contract shapes independently — it is meant to
// become a re-export of ../../../core/src/rpc.ts once WP-03 lands (see
// 01-plan.md 'Critical files' / the header comment below for the canonical
// source and the three-consumer sync rule). Until then this is a verbatim
// copy so the UI mock (mock/mock-sidecar.ts) and the real transport
// (app/transport.ts) both compile against the frozen G-RPC artefact per the
// WP-01 DoD ('a sidecar stub AND a UI mock both compile against it').
