...
// `host.invoke`. The host verbs it reaches are exactly three —
// `host.pkg.setMenu`, `host.pkgSidecarCall` (§0 of rpc.ts) and
// `host.navigate`. All audited below, nothing else.
//
// ── The seam that does NOT exist (R6) ───────────────────────────────────────
// WP-10 was written against a fourth verb, `host.sendToActiveSession`, on the
// strength of com.ikenga.studio calling it (`studio/src/studio/bridge.ts`).
// Verified against the shell: there is NO such case in the pkg-iframe
// dispatcher (`shell/src/components/pkg/pkg-iframe-host.tsx`) — the call falls
// through to 'unknown host tool'. Studio's helper is stale, not precedent. So
// "Send to your Chi" copies its prompt to the clipboard instead (app/
